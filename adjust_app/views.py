...
class PerformanceMetricsAPIView(APIView):
    ...
    def question_1_answer(self, date_from: str, date_to: str):
        ...
        if date_from and date_to:
            queryset = PerformanceMetrics.objects.filter(recorded_date__gte=date_from,
                                                         recorded_date__lte=date_to).values('channel',
                                                                                            'country').order_by(
                '-clicks').annotate(impressions=Sum('impressions'), clicks=Sum('clicks'))
            return queryset
        if date_from:
            queryset = PerformanceMetrics.objects.filter(recorded_date__gte=date_from).values('channel',
                                                                                              'country').order_by(
                '-clicks').annotate(impressions=Sum('impressions'), clicks=Sum('clicks'))
            return queryset
        if date_to:
            queryset = PerformanceMetrics.objects.filter(recorded_date__lte=date_to).values('channel',
                                                                                            'country').order_by(
                '-clicks').annotate(impressions=Sum('impressions'), clicks=Sum('clicks'))
            return queryset
    ...
    def question_2_answer(self, recorded_date: str, os: str):
        ...
        queryset = PerformanceMetrics.objects.filter(recorded_date__month=recorded_month,
                                                     recorded_date__year=recorded_year, os=os).values('installs',
    ...
    def question_3_answer(self, recorded_date: str, country: str):
        ...
        queryset = PerformanceMetrics.objects.filter(recorded_date__exact=recorded_date, country__exact=country).values(
            'revenue', 'os').order_by('-revenue')

        return queryset

    def question_4_answer_cpi_ca(self):

        queryset = PerformanceMetrics.objects.filter(country__exact='CA').values('channel').annotate(
            cpi=ExpressionWrapper(Sum(F('spend')) / Count(F('installs')), output_field=FloatField())).order_by('-cpi')
